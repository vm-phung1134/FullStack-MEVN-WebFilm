    ...
    <div class="container font-weight-light">
        <div class="my-md-2 p-md-5">
           <form class="shadow-content p-4 font-label" @submit.prevent="updateFilm()">
                <div>
                    <h4 class="text-cinema text-center mt-md-1 font-weight-light">CẬP NHẬT PHIM - PHIM ĐANG CHIẾU</h4>
...
<script>
    ...
    export default {
        ...
        async created() {
            const response = await API.findOne(this.$route.params.id);
            this.film = response;
        },
        methods: {
            async updateFilm(){
                try{
                    await API.update(this.$route.params.id, this.film);
            ...
            async deleteFilm(){
                try{
                    await API.delete(this.$route.params.id);
                    this.$router.push('/film-manager')
