...
exports.create = async (req, res, next) => {
    ...
    try {
        const checkoutModel = new CheckoutModel(MongoDB.client);
        const document = await checkoutModel.create(req.body);
...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        if (title) {
            documents = await checkoutModel.findByName(title);
        } else {
            documents = await checkoutModel.find({});
        }
    } catch (error) {
        return next(
...
exports.findOne = async (req, res, next) => {
    try {
        const checkoutModel = new CheckoutModel(MongoDB.client);
        const document = await checkoutModel.findById(req.params.id);
...
exports.update = async (req, res, next) => {
    ...
    try {
        const checkoutModel = new CheckoutModel(MongoDB.client);
        const document = await checkoutModel.update(req.params.id, req.body);
...
exports.delete = async (req, res, next) => {
    try {
        const checkoutModel = new CheckoutModel(MongoDB.client);
        const document = await checkoutModel.delete(req.params.id);
        if (!document) {
            return next(new ApiError(404, "film not found"));
        }
...
exports.deleteAll = async (req, res, next) => {
    try {
        const checkoutModel = new CheckoutModel(MongoDB.client);
        const deletedCount = await checkoutModel.deleteAll();
        return res.send({
