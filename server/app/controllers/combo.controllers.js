...
exports.create = async (req, res, next) => {
    ...
    try {
        const comboModel = new ComboModel(MongoDB.client);
        const document = await comboModel.create(req.body);
...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        if (title) {
            documents = await comboModel.findByName(title);
        } else {
            documents = await comboModel.find({});
        }
    } catch (error) {
        return next(
...
exports.findOne = async (req, res, next) => {
    try {
        const comboModel = new ComboModel(MongoDB.client);
        const document = await comboModel.findById(req.params.id);
...
exports.update = async (req, res, next) => {
    ...
    try {
        const comboModel = new ComboModel(MongoDB.client);
        const document = await comboModel.update(req.params.id, req.body);
...
exports.delete = async (req, res, next) => {
    try {
        const comboModel = new ComboModel(MongoDB.client);
        const document = await comboModel.delete(req.params.id);
        if (!document) {
            return next(new ApiError(404, "film not found"));
        }
...
exports.deleteAll = async (req, res, next) => {
    try {
        const comboModel = new ComboModel(MongoDB.client);
        const deletedCount = await comboModel.deleteAll();
        return res.send({
