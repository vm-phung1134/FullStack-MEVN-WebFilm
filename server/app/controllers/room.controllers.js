...
exports.create = async (req, res, next) => {
    ...
    try {
        const roomModel = new RoomModel(MongoDB.client);
        const document = await roomModel.create(req.body);
...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        if (title) {
            documents = await roomModel.findByName(title);
        } else {
            documents = await roomModel.find({});
        }
    } catch (error) {
        return next(
...
exports.findOne = async (req, res, next) => {
    try {
        const roomModel = new RoomModel(MongoDB.client);
        const document = await roomModel.findById(req.params.id);
...
exports.update = async (req, res, next) => {
    ...
    try {
        const roomModel = new RoomModel(MongoDB.client);
        const document = await roomModel.update(req.params.id, req.body);
...
exports.delete = async (req, res, next) => {
    try {
        const roomModel = new RoomModel(MongoDB.client);
        const document = await roomModel.delete(req.params.id);
        if (!document) {
            return next(new ApiError(404, "film not found"));
        }
...
exports.deleteAll = async (req, res, next) => {
    try {
        const roomModel = new RoomModel(MongoDB.client);
        const deletedCount = await roomModel.deleteAll();
        return res.send({
