...
exports.create = async (req, res, next) => {
    ...
    try {
        const cinemaModel = new CinemaModel(MongoDB.client);
        const document = await cinemaModel.create(req.body);
...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        if (title) {
            documents = await cinemaModel.findByName(title);
        } else {
            documents = await cinemaModel.find({});
        }
    } catch (error) {
        return next(
...
exports.findOne = async (req, res, next) => {
    try {
        const cinemaModel = new CinemaModel(MongoDB.client);
        const document = await cinemaModel.findById(req.params.id);
...
exports.update = async (req, res, next) => {
    ...
    try {
        const cinemaModel = new CinemaModel(MongoDB.client);
        const document = await cinemaModel.update(req.params.id, req.body);
...
exports.delete = async (req, res, next) => {
    try {
        const cinemaModel = new CinemaModel(MongoDB.client);
        const document = await cinemaModel.delete(req.params.id);
        if (!document) {
            return next(new ApiError(404, "film not found"));
        }
